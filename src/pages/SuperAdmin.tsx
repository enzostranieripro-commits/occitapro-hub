...
import { Shield, Users, Building2, Trash2, Ban, ArrowLeft, RefreshCw } from 'lucide-react';
...
export default function SuperAdminPage() {
  ...
  const SECTORS: Record<string, string> = {
    restauration: '🍽️ Restauration', btp: '🏗️ BTP',
    transport: '🚛 Transport', commerce: '🛒 Commerce',
    sante: '💆 Santé', immobilier: '🏡 Immobilier',
    tourisme: '🌿 Tourisme', hotellerie: '🏨 Hôtellerie',
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">Vous n'avez pas les droits super admin.</p>
          <Button className="mt-6" onClick={() => navigate('/')}>Retour</Button>
        </div>
      </div>
    );
  }
                ...
                <tr key={ws.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleToggleActive(ws)}>
                        <Ban className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="sm" className="text-destructive" onClick={() => handleDelete(ws.id, ws.name)}>
