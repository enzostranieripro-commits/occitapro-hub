...
function getStoredCreds() {
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed.login === 'string' && typeof parsed.password === 'string' && parsed.login.trim() !== '') {
        return parsed;
      }
    }
  } catch {
    // Si erreur de parsing, on supprime la clé corrompue
...
export default function LandingPage() {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    const storedLoginTrimmed = stored.login.trim();

    if (loginTrimmed === storedLoginTrimmed && password === stored.password) {
  ...
  const handleResetCreds = () => {
    ...
    setLogin('');
    setPassword('');
    alert('Identifiants réinitialisés. Utilisez : AveyronIA / Projet12');
  };
  ...
  const handleChangeCredentials = (e: React.FormEvent) => {
    ...
    setChangeSuccess('');
    const stored = getStoredCreds();
    if (currentLogin.trim() !== stored.login.trim() || currentPassword !== stored.password) {
                  ...
                  <form onSubmit={handleAdminLogin} className="space-y-4">
                    ...
                    {loginError && (
                      <div className="space-y-1">
                        <p className="rounded-lg bg-destructive/10 px-3 py-2 text-center text-sm text-destructive">{loginError}</p>
                        <button
                          type="button"
