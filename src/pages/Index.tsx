...
const DEFAULT_PASSWORD = 'enzo1512';
...
export default function LandingPage() {
  ...
  const handleChangeCredentials = (e: React.FormEvent) => {
    ...
    if (!newLogin.trim()) {
      setChangeError('Le nouvel identifiant ne peut pas être vide.');
      return;
    }
    ...
    const newCreds = { login: newLogin.trim(), password: newPassword };
    ...
    // Switch back to login tab after 1.5s
    setTimeout(() => {
      setActiveTab('login');
      setChangeSuccess('');
    }, 1500);
                ...
                {activeTab === 'change' && (
                  <form onSubmit={handleChangeCredentials} className="space-y-3">
                    ...
                    <div>
                      ...
                      <Input
                        value={currentLogin}
                        onChange={e => { setCurrentLogin(e.target.value); setChangeError(''); }}
                    ...
                    <div>
                      ...
                      <Input
                        value={newLogin}
                        onChange={e => { setNewLogin(e.target.value); setChangeError(''); }}
                    ...
                    {changeSuccess && (
                      <p className="rounded-lg bg-green-100 px-3 py-2 text-center text-sm text-green-800">
                        {changeSuccess}
                      </p>
                    )}
                    <Button type="submit" className="w-full gap-2">
